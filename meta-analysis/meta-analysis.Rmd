...
meta.data <- read_csv('all_data_unverified_for_testing.csv') %>% 
...
```{r}
fit <- reitsma(meta.data, correction.control='single')
...
ggplot() +
  ...
  geom_point(aes(fpr(meta.data), sens(meta.data)), size = 1) +
...
stain.fit <- reitsma(stain.data, correction.control='single')
...
frozen.fit <- reitsma(frozen.data, correction.control='single')
...
optical.fit <- reitsma(optical.data, correction.control='single')
...
chemo.fit <- reitsma(chemo.data, correction.control='single')
...
tic.fit <- reitsma(tic.data, correction.control='single')
...
auto.fit <- reitsma(auto.data, correction.control='single')
...
ggplot() +
  ...
  geom_point(data = frozen.summary_data, aes(V1, V2), shape = 21) +
